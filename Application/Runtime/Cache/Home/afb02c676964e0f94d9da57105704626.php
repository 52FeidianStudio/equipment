...
<html>
	<head>
		...
		<link rel="stylesheet" type='text/css' href="/equipment/Public/Css/communication.css" />
		...
		<script>
			...
			function f1(){
				$.post("/equipment/index.php/Home/Login/do_login", {
					...
					if(text=="pass"){
						window.location.href="/equipment/index.php/Home/Message/conmunication";
					}else{
						$("#suggest").html(text);
					}
				});
	...
	<body>
		...
		<div id='main'>
			<div class="contain">
				<div id="main_navi">
						<a href="/equipment/index.php/Home/Index/main">首页</a><span>--->讨论区 </span><br />
				...
				<div id="show">
					<?php if(($user) == "0"): ?><a href="/equipment/index.php/Home/Message/add_message"><p>添加留言</p></a><br />
							...
							<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><p><?php echo ($vo["title"]); ?><a href="/equipment/index.php/Home/Message/do_see/id/<?php echo ($vo["id"]); ?>">查看详情</a> <span id='time'> <?php echo ($vo["date"]); ?></span></p><?php endforeach; endif; else: echo "" ;endif; ?>
						<?php echo ($show); ?>
				</div>
			</div>
		</div>
